...
export const GAME_HELP = {
  ...
  'air-hockey': { steps: [['🏒', 'Faceoff', 'The puck stays at center until either player hits it with their paddle.'], ['●', 'Strike', 'Faster paddle impacts send the puck farther; angle your hit toward the opposite goal.'], ['⚑', 'Serve', 'After a goal, the puck rests on the other player’s half until struck.']], controls: 'Drag on the table; P1 (bottom) can use arrows, P2 (top) WASD.', options: 'Choose a winning score of 5, 7 or 10.', goal: 'Be the first to reach the chosen score.' },
  ...
  'crazy-eights': { steps: [['🂠', 'Deal', 'Keep your cards hidden during handoffs.'], ['8', 'Match', 'Play the same suit or rank; an eight changes the active suit.'], ['↗', 'Draw', 'Draw when you cannot play, then pass the device.']], controls: 'Reveal only your own hand, then tap a playable card or draw.', options: 'Two-player local pass and play; wild eights choose a new suit.', goal: 'Empty your hand first, or have fewer penalty points if blocked.' },
  ...
  uno: { steps: [['🌈', 'Match', 'Play a card matching the active color or top value.'], ['+2', 'Action', 'Skip, reverse and draw cards change the next turn.'], ['★', 'Wild', 'Choose a new color; +4 is only legal with no active-color card.']], controls: 'Reveal your hand locally; tap a playable card, or draw one and play only that card or pass.', options: '2–4 local or room players. Draw cards do not stack; +4 challenges, call-outs and multi-round scoring are not part of these house rules.', goal: 'Empty your hand; a blocked game goes to the fewest cards.' },
